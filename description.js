...
movieDesc.innerHTML = `
    ...
    <div> 
    <a href="checkout/checkout.html" class="btn btn-secondary" target="_blank">checkout</a>
    </div>
...
function toggleSeatSelection(seatIndex) {
  seat[seatIndex].classList.toggle('selected');
}
...
function bookSeats() {
  ...
  if (selectedSeats.length === 0) {
    ...
    let seatNumbers = selectedSeats.map(function(seat) {
      return Array.from(seat).indexOf(seat) + 1;
    });
